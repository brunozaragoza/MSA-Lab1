...
public:
  ...
  Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const {
    ...
    for (unsigned int i = 0; i < lights.size(); ++i) {
      ...
      // and compute the intersection
        Ray3f shadow_ray(its.p,emitterRecord.p);
        Intersection its2;
        scene->rayIntersect(shadow_ray,its2);
        if(its2.t>=emitterRecord.dist){
      ...
      BSDFQueryRecord bsdfRecord(its.toLocal(-ray.d),
                                 its.toLocal(emitterRecord.wi), its.uv,
                                 ESolidAngle);
  ...
  std::string toString() const { return "Direct Whitted Integrator []"; }
};
